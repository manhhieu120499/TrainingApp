...
function ChatItem(props) {
	let { name, url, message, numberOfUnreadMessage, timeUnreadMessage } =
		props.data;
	let { onPress } = props;
	return (
		<TouchableOpacity
		...
		>
			<View style={{ flexDirection: 'row', alignItems: 'center' }}>
				...
				<View style={{ marginStart: 18 }}>
					...
					<Text style={{ fontSize: fontSize.h6, color: '#888' }}>
						{message}
					</Text>
				</View>
			</View>
			<Text style={{ fontSize: fontSize.h7 }}>
				{convertTime(timeUnreadMessage)}
			</Text>
		</TouchableOpacity>
	);
}
