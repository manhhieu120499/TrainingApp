...
function FoodList(props) {
	...
	useEffect(() => console.log(inputFood));
	...
	const filteredFood = () => {
		return food.foodArr.filter((eachFood) =>
			eachFood.name.toLowerCase().includes(inputFood.toLowerCase())
		);
	};
	return (
		<View
		...
		>
			...
			<View style={{ flex: 60 }}>
				<FlatList
					data={food.foodArr.filter((eachFood) =>
						eachFood.name
							.toLowerCase()
							.includes(inputFood.toLowerCase())
					)}
					renderItem={(item) => (
						<FoodItem
							data={item}
