...
import {
	...
	getPopulation as PopulationRepository,
} from '../repositories';
...
function Profile(props) {
	...
	useEffect(() => {
		PopulationRepository().then((populationItems) => {
			console.log(populationItems);
			setPopulationInfo(populationItems);
		});
	}, []);
		...
		<View style={{ flex: 1, backgroundColor: 'green' }}>
			...
			<View style={{ flex: 1, backgroundColor: 'white' }}>
				...
				<View style={{ flex: 1, backgroundColor: 'green' }}>
					<Text>
						{JSON.stringify(
							populationInfo.map((item) => item).join(',')
						)}
					</Text>
				</View>
			</View>
